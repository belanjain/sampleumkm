...
function updateCounter(namaItem, harga, gridId, index) {
  ...
  if (qty === 0) {
    ...
    // Tampilkan counter +/- dengan input manual di tengah
    container.innerHTML = `
      <div class="qty-control">
        ...
        <input
          ...
          min="0"
          value="${qty}"
          id="input-${gridId}-${index}"
          onchange="setQtyManual('${safeName}', ${harga}, '${gridId}', ${index}, this.value)"
          onclick="this.select()"
        />
...
/**
 * Set qty secara manual dari input ketikan user
 * Kalau input 0 atau kosong → hapus dari cart
 */
function setQtyManual(namaItem, harga, gridId, index, nilai) {
  const qty = parseInt(nilai) || 0;

  if (qty <= 0) {
    delete cart[namaItem];
  } else {
    cart[namaItem] = { price: harga, qty };
  }
...
function updateCartBar() {
  ...
  const isVisible = totalItem > 0;
  cartBar.classList.toggle('visible', isVisible);

  // Geser floating WA ke atas biar gak nabrak cart bar di HP
  const waFloat = document.getElementById('wa-float');
  if (waFloat) {
    waFloat.style.bottom = isVisible ? '90px' : '28px';
  }
}
